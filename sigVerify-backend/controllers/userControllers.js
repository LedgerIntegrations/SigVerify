...
exports.createInitalUserTablesAndEmailAuthToken = async (
  req,
  res
) => {
  ...
  try {
    ...
    const returnedAuthTokenFromQueriedEmail = await client.query(
      queryIsEmailInDb,
      [email]
    );
    console.log(
      'db user query by email response: ',
      returnedAuthTokenFromQueriedEmail
    );

    //if email is in db that is not verified
    if (returnedAuthTokenFromQueriedEmail.rows.length > 0) {
      const userAuthToken =
        returnedAuthTokenFromQueriedEmail.rows[0].auth_token;
    ...
    const newAuthToken = crypto
      .randomBytes(SIGNUP_TOKEN_LENGTH)
      .toString('hex');
    ...
    try {
      ...
      console.log(
        'newUserAuthTableEntryId result: ',
        newUserAuthTableEntryId
      );
      ...
      console.log(
        'newUserMetaTableEntryId result: ',
        newUserMetaTableEntryId
      );

      await client.query(
        'INSERT INTO user_email (user_meta_id, email) VALUES ($1, $2) RETURNING id',
        [newUserMetaTableEntryId.rows[0].id, email]
      );
    } catch (err) {
      if (err.code === '23505') {
...
exports.createNewUser = async (req, res) => {
  const { FirstName, LastName, Password, PasswordConfirm, Token } =
    req.body;
  ...
  try {
    ...
    const authData = await client.query(
      'SELECT * FROM user_auth WHERE auth_token = $1',
      [Token]
    );
    ...
    await client.query(
      'UPDATE user_meta SET first_name = $1, last_name = $2 WHERE user_auth_id = $3',
      [FirstName, LastName, user_auth_id]
    );

    // Update user_auth table with the hashed password
    await client.query(
      'UPDATE user_auth SET hashed_password = $1 WHERE id = $2',
      [hashedPassword, user_auth_id]
    );

    // Verify user
    await client.query(
      'UPDATE user_auth SET auth_token = NULL WHERE id = $1',
      [user_auth_id]
    );
    ...
    const newUserDataQuery = `
      SELECT
          ...
          COUNT(DISTINCT s.id) AS total_signatures
      FROM
          user_auth AS ua
      JOIN
      ...
      LEFT JOIN
          documents AS d ON um.id = d.user_meta_id
      LEFT JOIN
          signatures AS s ON um.id = s.user_id
      WHERE
          ua.id = $1
      GROUP BY
          ua.id, um.first_name, ue.email, um.verified_xrpl_wallet_address, um.membership, um.document_limit;
    `;

    const newUserData = await client.query(newUserDataQuery, [
      user_auth_id,
    ]);
    ...
    return res.status(HTTP_OK).json({
      ...
      user: {
        ...
        email: user.email,
        verifiedXrplWalletAddress: user.verified_xrpl_wallet_address,
        membership: user.membership,
        documentLimit: user.document_limit,
        totalDocuments: user.total_documents,
        totalSignatures: user.total_signatures,
      },
    });
  } catch (err) {
    await client.query('ROLLBACK');
...
//       return res
//         .status(401)
//         .json({ error: 'Email not found.', loggedIn: false });
//     }
...
//         .json({
//           error:
//             'Email verified but account not created, follow link from email again.',
//           loggedIn: false,
//         });
//     }
...
//       return res
//         .status(401)
//         .json({ error: 'Invalid password.', loggedIn: false });
//     }
...
exports.authenticateLogin = async (req, res) => {
  ...
  try {
    ...
    const hashedEmail = await hashEmail(Email);
    console.log("hashed email in authenticateLogin: ", hashEmail)

    const userLoginResult = await client.query(loginQuery, [
      hashedEmail,
    ]);
