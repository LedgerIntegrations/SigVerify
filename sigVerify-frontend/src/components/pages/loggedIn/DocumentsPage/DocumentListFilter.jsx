...
const DocumentItem = styled.div`
    ...
    button {
        ...
        margin-top: 5px;
        box-shadow: inset 2px 2px 2px 0px rgba(255, 255, 255, 0.5), 2px 2px 7px 0px rgba(0, 0, 0, 0.1),
            2px 2px 7px 0px rgba(0, 0, 0, 0.1);

        &:hover {
            background-color: white;
            box-shadow: inset 1px 1px 2px 1px rgba(59, 59, 59, 0.5), 2px 2px 10px 0px rgba(0, 0, 0, 0.1),
...
const DocItemDetails = styled.ul`
    padding: 0;
    list-style: none;
`;

// eslint-disable-next-line react/prop-types
const DocumentListFilter = ({ options, documents }) => {
    ...
    const handlePrepareClick = (doc) => {
        navigate('/prepare', { state: { document: doc } });
    };
                ...
                {filteredDocuments?.map((doc, index) => (
                    <DocumentItem key={index}>
                        ...
                        <DocItemDetails>
                            ...
                            <li>
                                hash: <strong>{doc.xrplTxHash ? doc.xrplTxHash : 'false'}</strong>
                            </li>
                            <li>
                                expires: <strong>{doc.expires ? doc.expires : 'false'}</strong>
                            </li>
                            <li>
                                uploaded: <strong>{doc.uploaded ? formatDate(doc.uploaded) : 'null'}</strong>
                            </li>
                        </DocItemDetails>
                        <button onClick={() => handlePrepareClick(doc)}>Prepare</button>
                        <br />
                        <button onClick={() => handleDocumentClick(doc)}>View</button>
                    </DocumentItem>
                ))}
            </DocumentList>
